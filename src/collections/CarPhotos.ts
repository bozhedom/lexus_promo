...
export const CarPhotos: CollectionConfig = {
  ...
  admin: {
    ...
    description: {
      ru: 'Кадры автомобилей для пригласительных — весь каталог ведётся здесь. Кадр подбирается по марке, модели и году: чем точнее совпадение, тем выше приоритет. Поколения одной модели разводятся годом: у нового кадра годы пустые, ему же достаётся автомобиль с неизвестным годом.',
